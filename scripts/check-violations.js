...
// Windows 경로(\)를 POSIX(/)로 정규화 — 화이트리스트 정규식은 전부 / 기준이다
...
function checkCssFile(filePath) {
  ...
  const posixFile = posix(filePath)

  // 자동 생성 파일 제외
  if (lines[0] && (lines[0].includes('AUTO-GENERATED') || lines[0].includes('자동 생성'))) return

  // 토큰 정의 파일 / 프로젝트 스타일 확장 파일은 raw hex 허용 (정의 자체가 raw임)
  const isTokenFile = posixFile.includes('1-settings') ||
                       posixFile.includes('_project-style') ||
                       posixFile.includes('tokens/build')

  const isEntryFile = /\/src\/styles\/(?:style|docs)\.css$/.test(posixFile) ||
                      /\/src\/styles\/6-components\/index\.css$/.test(posixFile) ||
                      /\/src\/styles\/3-generic\/reset\.css$/.test(posixFile)
  ...
  lines.forEach((line, i) => {
    ...
    if (IMPORTANT_USED.test(trimmed) && !posixFile.includes('7-utilities')) {
...
function shouldSkipHtmlFile(filePath) {
  return HTML_SKIP_PATTERNS.some(re => re.test(posix(filePath)))
}
...
function checkHtmlFile(filePath) {
  ...
  const isPartial = /\/_includes\/partials\//.test(posixFile) || /\/_includes\/components\//.test(posixFile)
  const isPlayground = /\/src\/playground\//.test(posixFile)
  const isSnippet = /\/src\/snippets\//.test(posixFile)
